...
@dataclass
class RelationshipsManagerWindow(IComponent):
    x: int
    y: int
    width: int
    height: int
    ...
    def __post_init__(self):
        ...
        self.column = Column(self.x, self.y, column_w, self.height,
                             items=[relationship for relationship in self.model.relationships])
        btn_w, btn_h = 80, 25
        btn_y = self.y
    ...
    def start_edit(self):
        ...
        self.editing = True
        self.edit_index = idx
        tpl = self.model.relationships[idx]
    ...
    def delete_selected(self):
        ...
        if idx is None:
            return
        try:
            del self.model.relationships[idx]
        except Exception as exc:
            logging.error(f"Delete exchange error: {exc}")
        self.column.items = [relationship for relationship in self.model.relationships]
    ...
    def confirm_edit(self):
        ...
        if self.edit_index is None:
            tpl = name
            self.model.relationships.append(tpl)
        else:
            self.model.relationships[self.edit_index] = name
        self.column.items = [relationship for relationship in self.model.relationships]
        self.editing = False
        self.column.selected_index = None
